...
DMX_vec SpiderFixture::RGBW(simpleColor c, int intensity){
    ...
    case green:     return fcn::RGBW_norm(DMX_vec{0,255,0,0}, 160.0 /255*intensity);     break;
    ...
    case c_white:   return fcn::RGBW_norm(DMX_vec{0,0,0,255}, 200.0 /255*intensity);     break;
...
void SpiderAnimation2::new_frame(){
    ...
    // TODO add a functionality to set a range of tilit at cnstruction() & pick random tilts valuse within the range at init()   
    // Tilt
    ...
    // Colors
    bool beam_enabled = this->flash_length > 500 || sampler.state != BREAK;
